...
import { buildKeyFromHex, computeSig } from "../verifySignature"
import { type WebsocketRequest, Router as WsRouter } from "../wsRouter"

// ★ IDE 工作区签名验签中间件:对 ?folder=<path>&sig=<sig> 做 HMAC-SHA256 验签。
//   必须在所有下游逻辑前(包括 router.get("/") 和 catch-all 路由)。
//   签名不匹配或缺失 sig → 403,阻止用户篡改 folder 越权访问其他用户目录。
function verifySignatureMiddleware(req: express.Request, _res: express.Response, next: express.NextFunction): void {
  const key = buildKeyFromHex(req.args["signing-key"])
  if (!key) return next()
  const folder = req.query.folder
  const sig = req.query.sig
  if (!folder && !sig) return next()
  if (typeof folder !== "string" || typeof sig !== "string") {
    _res.status(403).send("无效的工作区标识")
    return
  }
  const expected = computeSig(folder, key)
  if (!crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expected))) {
    logger.warn("workspace sig verification failed", field("folder", folder), field("got", sig), field("expected", expected))
    _res.status(403).send("无效的工作区标识")
    return
  }
  next()
