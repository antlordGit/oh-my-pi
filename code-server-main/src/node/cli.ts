...
export interface UserProvidedCodeArgs {
  ...
  // 与后端 IdeService 共享的 HMAC-SHA256 key (64 hex 字符)，用于验证 ?folder=&sig= 签名
  "signing-key"?: string
...
export const options: Options<Required<UserProvidedArgs>> = {
  ...
  "signing-key": {
    type: "string",
    description:
      "64 个 hex 字符 (32 字节) 的 HMAC-SHA256 key。启用后,?folder= 的请求必须携带 &sig= 参数且签名匹配,\n" +
      "否则返回 403。空或未提供则跳过验签。",
  },
}
